...
long long **mulmat(long long **mat1, long long **mat2, int len){
	...
	for (int i = 0; i < len; i++) {
		tmp[i] = (long long*)malloc(sizeof(long long) * len);
		for (int j = 0; j < len; j++) {
			tmp[i][j] = 0;
		}
	}
...
int main(void) {
	...
	cin >> n >> m;
	mat = (long long**)malloc(sizeof(long long*) * n);
	for (int i = 0; i < n; i++) {
		mat[i] = (long long*)malloc(sizeof(long long) * n);
	}
	base = (long long**)malloc(sizeof(long long*) * n);
	for (int i = 0; i < n; i++) {
		base[i] = (long long*)malloc(sizeof(long long) * n);
	}
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			cin >> mat[i][j];
